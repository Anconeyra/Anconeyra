<div align="center">

# Frank Anconeyra

**`Full-Stack Developer · Cybersecurity · Cloud Architect`**

🏡 Arequipa, Perú · ✉️ anconeyrafsuyo@gmail.com · 💼 Open to work

[![Portfolio](https://img.shields.io/badge/Portfolio-255E63?style=for-the-badge&logo=about.me&logoColor=white)](https://anconeyra.github.io/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/frank-anconeyra)
[![Credly](https://img.shields.io/badge/Credly-FF6B6B?style=for-the-badge&logo=google-cloud&logoColor=white)](https://www.credly.com/users/frank-anconeyra)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:anconeyrafsuyo@gmail.com)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](https://wa.me/51917394464)

> *"I build systems that survive. Secure, scalable, cloud-native — from API to infrastructure."*

</div>

---

## 📊 Activity

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Anconeyra&theme=radical&hide_border=true&background=0d1117&stroke=764ba2&ring=667eea&fire=f093fb&currStreakLabel=764ba2&sideLabel=667eea&sideNums=c0caf5&currStreakNum=764ba2" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Anconeyra&theme=react-dark&color=667eea&line=764ba2&point=f093fb&hide_border=true&bg_color=0d1117&area=true&area_color=667eea20" alt="Activity Graph" />
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/Anconeyra/Anconeyra/output/github-contribution-grid-snake-dark.svg" alt="Snake animation" />
</p>

---

## 🚀 Shipped

<table>
  <tr>
    <td width="33%" valign="top">
      <h3>Greenfil</h3>
      <p><code>Flutter</code> <code>C#/.NET</code></p>
      <p>App industrial + backend para gestión de procesos. Full ownership del frontend mobile a la API.</p>
      <p>
        <a href="https://github.com/Anconeyra/Flutter-grenfil">Frontend</a> ·
        <a href="https://github.com/Anconeyra/C-backend-greenfi">Backend</a>
      </p>
    </td>
    <td width="33%" valign="top">
      <h3>LiteConta-SUNAT</h3>
      <p><code>PHP</code> <code>Blade</code></p>
      <p>Sistema contable con integración SUNAT. UX diseñada para minimizar la carga cognitiva del empresario peruano.</p>
      <p><a href="https://github.com/Anconeyra/LiteConta-SUNAT">Repositorio →</a></p>
    </td>
    <td width="33%" valign="top">
      <h3>MOSS</h3>
      <p><code>Python</code> <code>ML</code></p>
      <p>Pipeline de análisis y procesamiento de datos con técnicas de machine learning.</p>
      <p><a href="https://github.com/Anconeyra/MOSS">Repositorio →</a></p>
    </td>
  </tr>
  <tr>
    <td width="33%" valign="top">
      <h3>CRM Django</h3>
      <p><code>Python</code> <code>Django</code></p>
      <p>Customer Relationship Management con autenticación completa y CRUD.</p>
      <p><a href="https://github.com/Anconeyra/Crm-Django">Repositorio →</a></p>
    </td>
    <td width="33%" valign="top">
      <h3>Mynerz</h3>
      <p><code>Svelte</code></p>
      <p>Aplicación web moderna construida con Svelte.</p>
      <p><a href="https://github.com/Anconeyra/Mynerz">Repositorio →</a></p>
    </td>
    <td width="33%" valign="top">
      <h3>Gestión de Inventarios</h3>
      <p><code>HTML</code> <code>CSS</code> <code>JS</code></p>
      <p>Sistema de control y gestión de inventarios con interfaz web.</p>
      <p><a href="https://github.com/Anconeyra/Gesti-n-de-Inventarios">Repositorio →</a></p>
    </td>
  </tr>
</table>

<p align="right">
  <a href="https://github.com/Anconeyra?tab=repositories"><strong>→ All repositories</strong></a>
</p>

---

## 🛠️ Capabilities

| Area | Stack |
|------|-------|
| **Frontend** | React, Vue, Svelte, TypeScript, JavaScript, HTML/CSS |
| **Backend** | Java/Spring, Node.js, Python, Go, C#/.NET, PHP |
| **Mobile** | Flutter, Kotlin, Dart, Android |
| **Cloud & DevOps** | AWS, Azure, GCP, Docker, Kubernetes, Terraform, Jenkins, Postman |
| **Security** | OWASP, Wireshark, Kali Linux, Metasploit, Snyk, Vault, Elastic SIEM |
| **Databases** | PostgreSQL, MySQL, MongoDB, Redis, Cassandra, MariaDB, Firebase, SQL Server |
| **Embedded & IoT** | Arduino, OpenCV, Linux |

---

## 📜 Certifications

**15+** across Cisco (Cybersecurity, Networking, Python), AWS (Cloud, Serverless, Databases), SAP (SuccessFactors)

<p>
  <a href="https://www.credly.com/users/frank-anconeyra">
    <img src="https://img.shields.io/badge/View_Credly_Profile-FF6B6B?style=for-the-badge&logo=google-cloud&logoColor=white" alt="Credly" />
  </a>
</p>

---

<div align="center">
  <p>
    <a href="https://github.com/Anconeyra">GitHub</a> ·
    <a href="https://linkedin.com/in/frank-anconeyra">LinkedIn</a> ·
    <a href="mailto:anconeyrafsuyo@gmail.com">Email</a> ·
    <a href="https://anconeyra.github.io/">Portfolio</a> ·
    <a href="https://wa.me/51917394464">WhatsApp</a>
  </p>
  <p>
    <sub><em>Open for freelance & full-time opportunities</em></sub>
  </p>
</div>
